...
# Load ZTA roles
try:
    from shared.zta_roles import VALID_ROLE_NAMES, ZTA_ROLES
except ImportError:
    try:
...
def api_get_challenge():
    ...
    return jsonify({"challenge_id": challenge_id, "nonce_b64": nonce_b64})
...
def api_enroll_delegated():
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "Host": "localhost:9090"},
        method="POST",
    )
    ...
    try:
        ...
        return jsonify({"status": "error", "message": f"Delegation error: {e}"}), 500
...
def api_verify_identity():
    ...
    identity = service.verify_proof(
        challenge_id=challenge_id, signature_b64=signature_b64, public_key_pem=public_key_pem, proof_string=proof_string
    )
    if identity:
        return jsonify(
            {"status": "authenticated", "message": "Zero Trust hardware identity verified", "identity": identity}
        )
    else:
...
def api_get_roles():
    ...
    return jsonify({"roles": ZTA_ROLES, "valid_names": VALID_ROLE_NAMES})
