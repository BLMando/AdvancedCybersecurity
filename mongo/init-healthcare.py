...
client = MongoClient(
    args.uri, tls=True, tlsCertificateKeyFile=CLIENT_PEM_PATH, tlsCAFile=CA_PATH, tlsAllowInvalidCertificates=True
)
db = client[MONGO_DB]
...
for view in [
    "v_patients_doctor",
    "v_patients_billing",
    "v_patients_reception",
    "v_admissions_doctor",
    "v_admissions_reception",
    "v_admissions_billing",
...
db.command("create", "v_providers_all", viewOn="providers", pipeline=[{"$project": {"_id": 1, "type": 1, "name": 1}}])
...
roles_def = [
    ...
    },
    {
        ...
        "privileges": [
            ...
            {
                "resource": {"db": MONGO_DB, "collection": "v_clinical_auditor"},
                "actions": ["find"],
            },  # patient name -> initials
            {
                "resource": {"db": MONGO_DB, "collection": "v_billing_auditor"},
                "actions": ["find"],
            },  # amount rounded, provider masked
...
for role_doc in roles_def:
    try:
        ...
        pass
    db.command("createRole", role_doc["role"], privileges=role_doc["privileges"], roles=role_doc["roles"])
...
db_external.command("createUser", "CN=envoy,O=AdvancedCybersecurity-Clients,C=IT", roles=envoy_roles)
